...
async function createNewGroup(req, res, next) {
  try {
    filesUpload.uploadPic(req, res, function (err) {
      ...
      if (errors.indexOf(id) >= 0)
        return res.json({ status: false, msg: "Please provide the id." });
      if (errors.indexOf(selectedIDS) >= 0)
        return res.json({
          status: false,
          msg: "Please provide the selectedIDS.",
        });
      ...
      newGroup.save(function (err, response) {
        ...
        if (err == null)
          return res.json({ status: true, msg: "Group has been created" });
        else
          return res.json({
            status: false,
  ...
  try {
    filesUpload.uploadPic(req, res, function (err) {
      ...
      if (errors.indexOf(members) >= 0)
        return res.json({ status: false, msg: "Please provide the members." });
      if (errors.indexOf(groupId) >= 0)
        return res.json({ status: false, msg: "Please provide the groupId." });
      if (errors.indexOf(name) >= 0)
        return res.json({ status: false, msg: "Please provide the name." });
      if (errors.indexOf(total) >= 0)
        return res.json({ status: false, msg: "Please provide the total." });
      if (errors.indexOf(each) >= 0)
        return res.json({ status: false, msg: "Please provide the each." });
      ...
      for (let key of memberIDS) {
        ...
        newExpense.save(function (err, response) {
          ...
          if (cont == memberIDS.length) {
            if (err == null)
              return res.json({ status: true, msg: "Expense has been added" });
            else
              return res.json({
                status: false,
                msg: "Something Went Wrong. Please Try Again!",
