...
async function getLyncpayUsers(req, res, next) {
  try {
    const { myAllNumbers, myNumber } = req.body;
    if (errors.indexOf(myAllNumbers) >= 0)
      return res.json({
        status: false,
        msg: "Please provide the myAllNumbers.",
      });
    ...
    if (response.length != 0) {
      ...
      for (let key of response) {
        ...
        if (myNumber != key.phone) allContacts.push(dist);
        cont++;
  ...
  try {
    ...
    if (numbers.length != 0) {
      ...
      for (let key of numbers) {
        var isInvited = await contactInvitationTable.count({
          phone: key,
          senderId: userId,
        });
...
async function inviteContactOnLyncpay(req, res, next) {
  try {
    ...
    var doesExist = await contactInvitationTable.find({
      phone: final,
      senderId: userId,
    });

    if (doesExist.length != 0)
      return res.json({ status: false, msg: "Already invited!" });
    ...
    if (response.length != 0) {
      ...
      for (let key of response) {
        ...
        if (cont == response.length) {
          return res.json({
            status: true,
            data: arraySort(allContacts, "date", { reverse: true }),
          });
        }
      }
