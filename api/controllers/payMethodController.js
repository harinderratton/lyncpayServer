...
async function addNewCreditCard(req, res, next) {
  try {
    ...
    if (errors.indexOf(cardHolderName) >= 0)
      return res.json({
        status: false,
        msg: "Please provide the cardHolderName.",
      });
    if (errors.indexOf(cardNumber) >= 0)
      return res.json({ status: false, msg: "Please provide the cardNumber." });
    if (errors.indexOf(cardHolderEmail) >= 0)
      return res.json({
        status: false,
        msg: "Please provide the cardHolderEmail.",
      });
    ...
    if (errors.indexOf(customer) >= 0)
      return res.json({
        status: false,
        msg: "we Could not add this card at this moment.",
      });
    else saveCard();
    ...
    function saveCard() {
      ...
      newData.save(function (err, response) {
        if (err == null)
          return res.json({ status: true, msg: "New card is added" });
        else
          return res.json({
            status: false,
